'use strict';var util=require('../util/util');var l10n=require('../util/l10n');var settings=require('../settings');var Eagerness={NEVER:1,SOMETIMES:2,ALWAYS:3};exports.items=[{item:'setting',name:'eagerHelper',type:{name:'selection',lookup:[{name:'never',value:Eagerness.NEVER},{name:'sometimes',value:Eagerness.SOMETIMES},{name:'always',value:Eagerness.ALWAYS}]},defaultValue:Eagerness.SOMETIMES,description:l10n.lookup('eagerHelperDesc'),ignoreTypeDifference:true}];function FocusManager(document){if(document==null){throw new Error('document == null');}
this.document=document;this.debug=false;this.blurDelay=150;this.window=this.document.defaultView;this._blurDelayTimeout=null; this._monitoredElements=[];this._isError=false;this._hasFocus=false;this._helpRequested=false;this._recentOutput=false;this.onVisibilityChange=util.createEvent('FocusManager.onVisibilityChange');this._focused=this._focused.bind(this);if(this.document.addEventListener){this.document.addEventListener('focus',this._focused,true);}
var eagerHelper=settings.getSetting('eagerHelper');eagerHelper.onChange.add(this._eagerHelperChanged,this);this.isTooltipVisible=undefined;this.isOutputVisible=undefined;this._checkShow();}
FocusManager.prototype.destroy=function(){var eagerHelper=settings.getSetting('eagerHelper');eagerHelper.onChange.remove(this._eagerHelperChanged,this);this.document.removeEventListener('focus',this._focused,true);for(var i=0;i<this._monitoredElements.length;i++){var monitor=this._monitoredElements[i];console.error('Hanging monitored element: ',monitor.element);monitor.element.removeEventListener('focus',monitor.onFocus,true);monitor.element.removeEventListener('blur',monitor.onBlur,true);}
if(this._blurDelayTimeout){this.window.clearTimeout(this._blurDelayTimeout);this._blurDelayTimeout=null;}
this._focused=undefined;this.document=undefined;this.window=undefined;};FocusManager.prototype.addMonitoredElement=function(element,where){if(this.debug){console.log('FocusManager.addMonitoredElement('+(where||'unknown')+')');}
...
this.onVisibilityChange(ev);}};FocusManager.prototype._shouldShowTooltip=function(){var eagerHelper=settings.getSetting('eagerHelper');if(eagerHelper.value===Eagerness.NEVER){return{visible:false,reason:'eagerHelperNever'};}
