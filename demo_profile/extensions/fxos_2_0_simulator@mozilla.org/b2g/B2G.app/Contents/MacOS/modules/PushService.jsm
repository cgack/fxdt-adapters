...
const Cc=Components.classes;const Ci=Components.interfaces;const Cu=Components.utils;const Cr=Components.results;Cu.import("resource://gre/modules/XPCOMUtils.jsm");Cu.import("resource://gre/modules/Services.jsm");Cu.import("resource://gre/modules/IndexedDBHelper.jsm");Cu.import("resource://gre/modules/Timer.jsm");Cu.import("resource://gre/modules/Preferences.jsm");Cu.import("resource://gre/modules/Promise.jsm");Cu.importGlobalProperties(["indexedDB"]);XPCOMUtils.defineLazyModuleGetter(this,"AlarmService","resource://gre/modules/AlarmService.jsm");this.EXPORTED_SYMBOLS=["PushService"];const prefs=new Preferences("services.push.");gDebuggingEnabled=prefs.get("debug");const kPUSHDB_DB_NAME="push";const kPUSHDB_DB_VERSION=1;const kPUSHDB_STORE_NAME="push";const kUDP_WAKEUP_WS_STATUS_CODE=4774;
...
debug("shutdown complete!");},_reconnectAfterBackoff:function(){debug("reconnectAfterBackoff()");let retryTimeout=prefs.get("retryBaseInterval")*Math.pow(2,this._retryFailCount);retryTimeout=Math.min(retryTimeout,prefs.get("pingInterval"));this._retryFailCount++;debug("Retry in "+retryTimeout+" Try number "+this._retryFailCount);this._setAlarm(retryTimeout);},_beginWSSetup:function(){debug("beginWSSetup()");if(this._currentState!=STATE_SHUT_DOWN){debug("_beginWSSetup: Not in shutdown state! Current state "+
...
this._retryFailCount=0;this._listenForUDPWakeup();let data={messageType:"hello",}
if(this._UAID)
data["uaid"]=this._UAID;let networkState=this._getNetworkState();if(networkState.ip){data["wakeup_hostport"]={ip:networkState.ip,port:this._udpServer&&this._udpServer.port};data["mobilenetwork"]={mcc:networkState.mcc,mnc:networkState.mnc};}
function sendHelloMessage(ids){data["channelIDs"]=ids.map?ids.map(function(el){return el.channelID;}):[];this._wsSendMessage(data);this._currentState=STATE_WAITING_FOR_HELLO;}
...
this._shutdownWS();},_wsOnMessageAvailable:function(context,message){debug("wsOnMessageAvailable() "+message);this._waitingForPong=false;
this._setAlarm(prefs.get("pingInterval"));let reply=undefined;try{reply=JSON.parse(message);}catch(e){debug("Parsing JSON failed. text : "+message);return;}
if(typeof reply.messageType!="string"){debug("messageType not a string "+reply.messageType);return;}
...
let clientId=0;let iccInfo=icc.getIccInfo(clientId);if(iccInfo){debug("Running on mobile data");return{mcc:iccInfo.mcc,mnc:iccInfo.mnc,ip:nm.active.ip}}}}catch(e){}
debug("Running on wifi");return{mcc:0,mnc:0,ip:undefined};},_getNetworkStateChangeEventName:function(){try{Cc["@mozilla.org/network/manager;1"].getService(Ci.nsINetworkManager);return"network-active-changed";}catch(e){return"network:offline-status-changed";}}}