...
"use strict";module.metadata={"stability":"unstable"};const{deprecateFunction}=require("../util/deprecate");const{setImmediate,setTimeout,clearTimeout}=require("../timers");const arity=f=>f.arity||f.length;const name=f=>f.displayName||f.name;const derive=(f,source)=>{f.displayName=name(source);f.arity=arity(source);return f;};const method=(...lambdas)=>{return function method(...args){args.unshift(this);return lambdas.reduce((_,lambda)=>lambda.apply(this,args),void(0));};};exports.method=method;const defer=f=>derive(function(...args){setImmediate(invoke,f,args,this);},f);exports.defer=defer;exports.remit=defer;const invoke=(callee,params,self)=>callee.apply(self,params);exports.invoke=invoke;const partial=(f,...curried)=>{if(typeof(f)!=="function")
...
memo[key]=f.apply(this,args);return memo[key];}},f);};exports.memoize=memoize;const delay=function delay(f,ms,...args){setTimeout(()=>f.apply(this,args),ms);};exports.delay=delay;const once=f=>{let ran=false,cache;return derive(function(...args){return ran?cache:(ran=true,cache=f.apply(this,args));},f);};exports.once=once;exports.cache=once;
