...
const{validateOptions,validateSingleOption}=new OptionsValidator({url:{ map:function(url)url.toString(),ok:isValidURI},headers:{map:function(v)v||{},is:["object"],},content:{map:function(v)v||null,is:["string","object","null"],},contentType:{map:function(v)v||"application/x-www-form-urlencoded",is:["string"],},overrideMimeType:{map:function(v)v||null,is:["string","null"],}});const REUSE_ERROR="This request object has been used already. You must "+"create a new one to make a new request."

function runRequest(mode,target){let source=request(target)
let{xhr,url,content,contentType,headers,overrideMimeType}=source;let isGetOrHead=(mode=="GET"||mode=="HEAD");if(xhr)
throw new Error(REUSE_ERROR);xhr=source.xhr=new XMLHttpRequest();
