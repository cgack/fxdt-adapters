...
const XMLURI_PARSE_ERROR="http://www.mozilla.org/newlayout/xml/parsererror.xml";const API_VERSION="1.5";const DEFAULT_CACHE_TYPES="extension,theme,locale,dictionary";const KEY_PROFILEDIR="ProfD";const FILE_DATABASE="addons.json";const DB_SCHEMA=5;const DB_MIN_JSON_SCHEMA=5;const DB_BATCH_TIMEOUT_MS=50;const BLANK_DB=function(){return{addons:new Map(),schema:DB_SCHEMA};}
...
return enabled;}, _addons:null, _pendingCallbacks:null, _searching:false, _request:null,_callback:null, _maxResults:null,shutdown:function AddonRepo_shutdown(){this.cancelSearch();this._addons=null;this._pendingCallbacks=null;return AddonDatabase.shutdown(false);},getCachedAddonByID:Task.async(function*(aId,aCallback){if(!aId||!this.cacheEnabled){aCallback(null);return;}
let self=this;function getAddon(aAddons){aCallback((aId in aAddons)?aAddons[aId]:null);}
if(this._addons==null){if(this._pendingCallbacks==null){ this._pendingCallbacks=[];this._pendingCallbacks.push(getAddon);let addons=yield AddonDatabase.retrieveStoredData();let pendingCallbacks=self._pendingCallbacks; if(pendingCallbacks==null)
return;
 self._pendingCallbacks=null;self._addons=addons;pendingCallbacks.forEach(function(aCallback)aCallback(addons));return;} 
this._pendingCallbacks.push(getAddon);return;} 
getAddon(this._addons);}),repopulateCache:function(aIds,aCallback,aTimeout){this._repopulateCacheInternal(aIds,aCallback,false,aTimeout);},_repopulateCacheInternal:function(aIds,aCallback,aSendPerformance,aTimeout){ function repopulateAddonManager(){AddonManagerPrivate.updateAddonRepositoryData(aCallback);}
logger.debug("Repopulate add-on cache with "+aIds.toSource()); if(!this.cacheEnabled){logger.debug("Clearing cache because it is disabled");this._addons=null;this._pendingCallbacks=null;AddonDatabase.delete(repopulateAddonManager);return;}
let self=this;getAddonsToCache(aIds,function repopulateCache_getAddonsToCache(aAddons){ if(aAddons.length==0){logger.debug("Clearing cache because 0 add-ons were requested");self._addons=null;self._pendingCallbacks=null;AddonDatabase.delete(repopulateAddonManager);return;}
self._beginGetAddons(aAddons,{searchSucceeded:function repopulateCacheInternal_searchSucceeded(aAddons){self._addons={};aAddons.forEach(function(aAddon){self._addons[aAddon.id]=aAddon;});AddonDatabase.repopulate(aAddons,repopulateAddonManager);},searchFailed:function repopulateCacheInternal_searchFailed(){logger.warn("Search failed when repopulating cache");repopulateAddonManager();}},aSendPerformance,aTimeout);});},cacheAddons:function AddonRepo_cacheAddons(aIds,aCallback){logger.debug("cacheAddons: enabled "+this.cacheEnabled+" IDs "+aIds.toSource());if(!this.cacheEnabled){if(aCallback)
aCallback();return;}
let self=this;getAddonsToCache(aIds,function cacheAddons_getAddonsToCache(aAddons){ if(aAddons.length==0){if(aCallback)
aCallback();return;}
self.getAddonsByIDs(aAddons,{searchSucceeded:function cacheAddons_searchSucceeded(aAddons){aAddons.forEach(function(aAddon){self._addons[aAddon.id]=aAddon;});AddonDatabase.insertAddons(aAddons,aCallback);},searchFailed:function cacheAddons_searchFailed(){logger.warn("Search failed when adding add-ons to cache");if(aCallback)
...
this.jsonFile,error)).then(()=>this._deleting=null).then(aCallback);},toJSON:function AD_toJSON(){let json={schema:this.DB.schema,addons:[]}
...
return this.Writer.flush();},retrieveStoredData:Task.async(function*(){let db=yield this.openConnection();let result={};for(let[key,value]of db.addons){result[key]=value;}
return result;}),repopulate:function AD_repopulate(aAddons,aCallback){this.DB.addons.clear();this.insertAddons(aAddons,aCallback);},insertAddons:Task.async(function*(aAddons,aCallback){yield this.openConnection();yield this._insertAddons(aAddons,aCallback);}),_insertAddons:Task.async(function*(aAddons,aCallback){for(let addon of aAddons){this._insertAddon(addon);}
