...
function FFDisplay(options){if(options.eval){cli.setEvalFunction(options.eval);}
setContentDocument(options.contentDocument);this.commandOutputManager=options.commandOutputManager;if(this.commandOutputManager==null){this.commandOutputManager=new CommandOutputManager();}
this.onOutput=this.commandOutputManager.onOutput;this.requisition=new Requisition({environment:options.environment,document:options.outputDocument,commandOutputManager:this.commandOutputManager});this.focusManager=new FocusManager(options.chromeDocument);this.onVisibilityChange=this.focusManager.onVisibilityChange;this.inputter=new Inputter(options,{requisition:this.requisition,focusManager:this.focusManager,element:options.inputElement});this.completer=new Completer({requisition:this.requisition,inputter:this.inputter,backgroundElement:options.backgroundElement,element:options.completeElement});this.tooltip=new Tooltip(options,{requisition:this.requisition,focusManager:this.focusManager,inputter:this.inputter,element:options.hintElement});this.inputter.tooltip=this.tooltip;if(options.consoleWrap){this.resizer=this.resizer.bind(this);this.consoleWrap=options.consoleWrap;var win=options.consoleWrap.ownerDocument.defaultView;win.addEventListener('resize',this.resizer,false);}
this.options=options;}
FFDisplay.prototype.maybeShowIntro=function(){intro.maybeShowIntro(this.commandOutputManager,this.requisition.conversionContext);};FFDisplay.prototype.reattach=function(options){setContentDocument(options.contentDocument);};FFDisplay.prototype.destroy=function(){if(this.consoleWrap){var win=this.options.consoleWrap.ownerDocument.defaultView;win.removeEventListener('resize',this.resizer,false);}
