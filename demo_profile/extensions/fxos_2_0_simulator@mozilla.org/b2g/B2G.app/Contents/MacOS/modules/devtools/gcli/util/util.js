...
eventCombiner=newEventCombiner;},enumerable:true});return event;};var promise=require('./promise');exports.promiseEach=function(array,action,scope){if(array.length===0){return promise.resolve([]);}
var deferred=promise.defer();var replies=[];var callNext=function(index){var onSuccess=function(reply){replies[index]=reply;if(index+1>=array.length){deferred.resolve(replies);}
else{callNext(index+1);}};var onFailure=function(ex){deferred.reject(ex);};var reply=action.call(scope,array[index],index,array);promise.resolve(reply).then(onSuccess).then(null,onFailure);};callNext(0);return deferred.promise;};exports.errorHandler=function(ex){if(ex instanceof Error){ if(ex.stack.indexOf(ex.message)!==-1){console.error(ex.stack);}
...
catch(ex){console.error('Bad XHTML',ex);console.trace();throw ex;}}};exports.toDom=function(document,html){var div=exports.createElement(document,'div');exports.setContents(div,html);return div.children[0];};exports.isXmlDocument=function(doc){doc=doc||document; if(doc.contentType&&doc.contentType!='text/html'){return true;} 
if(doc.xmlVersion!=null){return true;}
return false;};function positionInNodeList(element,nodeList){for(var i=0;i<nodeList.length;i++){if(element===nodeList[i]){return i;}}
return-1;}
exports.createEmptyNodeList=function(doc){if(doc.createDocumentFragment){return doc.createDocumentFragment().childNodes;}
return doc.querySelectorAll('x>:root');};exports.findCssSelector=function(ele){var document=ele.ownerDocument;if(ele.id&&document.getElementById(ele.id)===ele){return'#'+ele.id;} 
...
index=positionInNodeList(ele,ele.parentNode.children)+1;selector=exports.findCssSelector(ele.parentNode)+' > '+
tagName+':nth-child('+index+')';return selector;};if(typeof'KeyEvent'==='undefined'){exports.KeyEvent=this.KeyEvent;}
