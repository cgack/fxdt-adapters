...
const nodeResolve=iced(function nodeResolve(id,requirer,{manifest,rootURI}){ id=exports.resolve(id,requirer);let fullId=join(rootURI,id);let resolvedPath;if(resolvedPath=loadAsFile(fullId))
return stripBase(rootURI,resolvedPath);else if(resolvedPath=loadAsDirectory(fullId))
return stripBase(rootURI,resolvedPath);
 else if(manifest.dependencies){let dirs=getNodeModulePaths(dirname(join(rootURI,requirer))).map(dir=>join(dir,id));for(let i=0;i<dirs.length;i++){if(resolvedPath=loadAsFile(dirs[i]))
...
const Loader=iced(function Loader(options){let{modules,globals,resolve,paths,rootURI,manifest,requireMap,isNative}=override({paths:{},modules:{},globals:{console:console},resolve:options.isNative?exports.nodeResolve:exports.resolve,},options);
