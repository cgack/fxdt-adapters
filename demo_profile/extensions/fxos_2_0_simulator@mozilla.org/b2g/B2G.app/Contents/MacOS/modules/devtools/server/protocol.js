...
return value;}));}
let Request=Class({initialize:function(template={}){this.type=template.type;this.template=template;this.args=findPlaceholders(template,Arg);},write:function(fnArgs,ctx){let str=JSON.stringify(this.template,(key,value)=>{if(value instanceof Arg){return value.write(fnArgs[value.index],ctx,key);}
...
let deferred=this._requests.shift();if(packet.error){deferred.reject(packet.error);}else{deferred.resolve(packet);}}});exports.Front=Front;exports.preEvent=function(eventName,fn){fn._preEvent=eventName;return fn;}
...
exports.dumpProtocolSpec=function(){let ret={types:{},};for(let[name,type]of registeredTypes){type=types.getType(name);if(type.category==="dict"){ret.types[name]={category:"dict",typeName:name,specializations:type.specializations}}else if(type.category==="actor"){ret.types[name]=exports.dumpActorSpec(type);}}
return ret;}