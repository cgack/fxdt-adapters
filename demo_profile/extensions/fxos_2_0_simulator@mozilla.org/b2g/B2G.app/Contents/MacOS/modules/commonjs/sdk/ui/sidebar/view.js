...
exports.updateTitle=updateTitle;function updateURL(sidebar,url){let eleID=makeID(sidebar.id);for(let window of windows(null,{includePrivate:true})){ let mi=window.document.getElementById(eleID);if(mi){mi.setAttribute('sidebarurl',url)} 
...
if(sidebarTitle.value==modelFor(sidebar).title){ if(win.gWebPanelURI==modelFor(sidebar).url){return true;} 
let ele=sb.contentDocument&&sb.contentDocument.getElementById(WEB_PANEL_BROWSER_ID);if(!ele){return false;}
if(ele.getAttribute('cachedurl')==modelFor(sidebar).url){return true;}
if(ele&&ele.contentWindow&&ele.contentWindow.location==modelFor(sidebar).url){return true;}} 
return false;}
...
else{sidebar.once('show',resolve);let menuitem=window.document.getElementById(makeID(model.id));menuitem.setAttribute('checked',true);window.openWebPanel(model.title,newURL||model.url);}
return promise;}
