"use strict";const{Cc,Ci,Cu,components}=require("chrome");Cu.import("resource://gre/modules/XPCOMUtils.jsm");loader.lazyImporter(this,"Services","resource://gre/modules/Services.jsm");loader.lazyImporter(this,"LayoutHelpers","resource://gre/modules/devtools/LayoutHelpers.jsm");loader.lazyImporter(this,"gDevTools","resource:///modules/devtools/gDevTools.jsm");loader.lazyImporter(this,"devtools","resource://gre/modules/devtools/Loader.jsm");loader.lazyImporter(this,"VariablesView","resource:///modules/devtools/VariablesView.jsm");loader.lazyImporter(this,"DevToolsUtils","resource://gre/modules/devtools/DevToolsUtils.jsm");const REGEX_MATCH_FUNCTION_NAME=/^\(?function\s+([^(\s]+)\s*\(/;const REGEX_MATCH_FUNCTION_ARGS=/^\(?function\s*[^\s(]*\s*\((.+?)\)/;let WebConsoleUtils={unwrap:function WCU_unwrap(aObject)
...
{return aGrip&&typeof(aGrip)=="object"&&aGrip.actor;},};exports.Utils=WebConsoleUtils;
...
return result;},};
(function _JSPP(WCU){const STATE_NORMAL=0;const STATE_QUOTE=2;const STATE_DQUOTE=3;const OPEN_BODY="{[(".split("");const CLOSE_BODY="}])".split("");const OPEN_CLOSE_BODY={"{":"}","[":"]","(":")",};const MAX_COMPLETIONS=1500;function findCompletionBeginning(aStr)
...
{let matches=new Set();let iter=chainIterator(aObj);for(let obj of iter){let props=getProperties(obj);for(let prop of props){if(prop.indexOf(aMatch)!=0){continue;}

if(+prop!=+prop){matches.add(prop);}
if(matches.size>MAX_COMPLETIONS){break;}}
if(matches.size>MAX_COMPLETIONS){break;}}
...
{let result=undefined;try{result=aObj.getVariable(aName);}catch(ex){}
...
return nodes;};Object.defineProperty(aOwner.sandbox,"$0",{get:function(){let window=aOwner.chromeWindow();if(!window){return null;}
let target=null;try{target=devtools.TargetFactory.forTab(window.gBrowser.selectedTab);}
catch(ex){
}
if(!target){return null;}
let toolbox=gDevTools.getToolbox(target);let node=toolbox&&toolbox.selection?toolbox.selection.node:null;return node?aOwner.makeDebuggeeValue(node):null;},enumerable:true,configurable:false});aOwner.sandbox.clear=function JSTH_clear()
...
{aOwner.helperResult={rawOutput:true};return String(aString);};}
