const EXPORTED_SYMBOLS=["BackgroundPageThumbs",];const DEFAULT_CAPTURE_TIMEOUT=30000;const DESTROY_BROWSER_TIMEOUT=60000;const FRAME_SCRIPT_URL="chrome://global/content/backgroundPageThumbsContent.js";const TELEMETRY_HISTOGRAM_ID_PREFIX="FX_THUMBNAILS_BG_";const TEL_CAPTURE_DONE_OK=0;const TEL_CAPTURE_DONE_TIMEOUT=1;const TEL_CAPTURE_DONE_CRASHED=4;const XUL_NS="http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul";const HTML_NS="http://www.w3.org/1999/xhtml";const{classes:Cc,interfaces:Ci,utils:Cu}=Components;Cu.import("resource://gre/modules/PageThumbs.jsm");Cu.import("resource://gre/modules/Services.jsm");const BackgroundPageThumbs={capture:function(url,options={}){if(!PageThumbs._prefEnabled()){if(options.onDone)
...
PageThumbsStorage.fileExistsForURL(url).then(exists=>{if(exists.ok){if(options.onDone)
...
delete this.captureCallback;delete this.doneCallbacks;delete this.options;},receiveMessage:function(msg){tel("CAPTURE_SERVICE_TIME_MS",new Date()-this.startDate);
if(msg.json.id==this.id)
this._done(msg.json,TEL_CAPTURE_DONE_OK);},notify:function(){this._done(null,TEL_CAPTURE_DONE_TIMEOUT);},_done:function(data,reason){
