...
var self=this;Object.defineProperty(commandParamMetadata,'description',{get:function(){var value=self.value;return value&&value.description?value.description:'The command to execute';},enumerable:true});this.param=new Parameter(requisition.types,commandParamMetadata);}
...
UnassignedAssignment.prototype=Object.create(Assignment.prototype);UnassignedAssignment.prototype.getStatus=function(arg){return this.conversion.getStatus();};var logErrors=true;Object.defineProperty(exports,'logErrors',{get:function(){return logErrors;},set:function(val){logErrors=val;},enumerable:true});function Requisition(options){options=options||{};this.environment=options.environment||{};this.document=options.document;if(this.document==null){try{this.document=document;}
catch(ex){}}
this.commandOutputManager=options.commandOutputManager||new CommandOutputManager();this.canon=options.canon||centralCanon;this.types=options.types||centralTypes;this.shell={cwd:'/', env:{}
...
return false;},this);return reply;};Requisition.prototype.getAssignmentAt=function(cursor){if(!this._args){console.trace();throw new Error('Missing args');}
...
var reply=assignForPos[cursor-1];if(!reply){throw new Error('Missing assignment.'+' cursor='+cursor+' text='+this.toString());}
return reply;};Requisition.prototype.toCanonicalString=function(){var cmd=this.commandAssignment.value?this.commandAssignment.value.name:this.commandAssignment.arg.text; var lineSuffix='';if(cmd==='{'){var scriptSuffix=this.getAssignment(0).arg.suffix;lineSuffix=(scriptSuffix.indexOf('}')===-1)?' }':'';}
...
return command;}
Requisition.prototype.update=function(typed){if(typeof HTMLElement!=='undefined'&&typed instanceof HTMLElement){typed=getDataCommandAttribute(typed);}
if(typeof Event!=='undefined'&&typed instanceof Event){typed=getDataCommandAttribute(typed.currentTarget);}
...
var conversion;if(args[0].type==='ScriptArgument'){
 conversion=new Conversion(getEvalCommand(this.canon),new ScriptArgument());this._setAssignmentInternal(this.commandAssignment,conversion);return;}
...
command=this.canon.getCommand(options.command);if(!command){console.error('Command not found: '+options.command);}
args=options.args;}}
...
var output=new Output({command:command,args:args,typed:typed,canonical:this.toCanonicalString(),hidden:hidden});this.commandOutputManager.onOutput({output:output});var onDone=function(data){output.complete(data,false);return output;};var onError=function(data,ex){if(logErrors){if(ex!=null){util.errorHandler(ex);}
...
else{try{var reply=command.exec(args,this.executionContext);return promise.resolve(reply).then(onDone,onError);}
catch(ex){var data=(typeof ex.message==='string'&&ex.stack!=null)?ex.message:ex;return promise.resolve(onError(data,ex));}
finally{this.clear();}}};Requisition.prototype.updateExec=function(input,options){return this.update(input).then(function(){return this.exec(options);}.bind(this));};exports.Requisition=Requisition;function Output(options){options=options||{};this.command=options.command||'';this.args=options.args||{};this.typed=options.typed||'';this.canonical=options.canonical||'';this.hidden=options.hidden===true?true:false;this.type=undefined;this.data=undefined;this.completed=false;this.error=false;this.start=new Date();this._deferred=promise.defer();this.promise=this._deferred.promise;}
...
this._deferred.resolve();};Output.prototype.convert=function(type,conversionContext){return converters.convert(this.data,this.type,type,conversionContext);};Output.prototype.toJson=function(){return{typed:this.typed,type:this.type,data:this.data,error:this.error};};exports.Output=Output;