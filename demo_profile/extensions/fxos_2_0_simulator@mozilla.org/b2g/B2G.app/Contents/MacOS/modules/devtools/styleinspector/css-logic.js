const{Cc,Ci,Cu}=require("chrome");const RX_UNIVERSAL_SELECTOR=/\s*\*\s*/g;const RX_NOT=/:not\((.*?)\)/g;const RX_PSEUDO_CLASS_OR_ELT=/(:[\w-]+\().*?\)/g;const RX_CONNECTORS=/\s*[\s>+~]\s*/g;const RX_ID=/\s*#\w+\s*/g;const RX_CLASS_OR_ATTRIBUTE=/\s*(?:\.\w+|\[.+?\])\s*/g;const RX_PSEUDO=/\s*:?:([\w-]+)(\(?\)?)\s*/g;Cu.import("resource://gre/modules/Services.jsm");Cu.import("resource://gre/modules/XPCOMUtils.jsm");function CssLogic()
{_propertyInfos:{};}
...
_passId:0,_matchId:0,_matchedRules:null,_matchedSelectors:null,reset:function CssLogic_reset()
{this._propertyInfos={};this._ruleCount=0;this._sheetIndex=0;this._sheets={};this._sheetsCached=false;this._matchedRules=null;this._matchedSelectors=null;},highlight:function CssLogic_highlight(aViewedElement)
...
let cssSheet=this.getSheet(aDomSheet,this._sheetIndex++);if(cssSheet._passId!=this._passId){cssSheet._passId=this._passId;Array.prototype.forEach.call(aDomSheet.cssRules,function(aDomRule){if(aDomRule.type==Ci.nsIDOMCSSRule.IMPORT_RULE&&aDomRule.styleSheet&&this.mediaMatches(aDomRule)){this._cacheSheet(aDomRule.styleSheet);}},this);}},get sheets()
{if(!this._sheetsCached){this._cacheSheets();}
let sheets=[];this.forEachSheet(function(aSheet){if(aSheet.contentSheet){sheets.push(aSheet);}},this);return sheets;},getSheet:function CL_getSheet(aDomSheet,aIndex)
...
{if(!this._matchedRules){this._buildMatchedRules();}
let result={};this._matchedRules.some(function(aValue){let rule=aValue[0];let status=aValue[1];aProperties=aProperties.filter(function(aProperty){
...
return true;}.bind(this));return aProperties.length==0;},this);return result;},_buildMatchedRules:function CL__buildMatchedRules()
...
return selectors;}
CssLogic.l10n=function(aName)CssLogic._strings.GetStringFromName(aName);XPCOMUtils.defineLazyGetter(CssLogic,"_strings",function()Services.strings.createBundle("chrome://global/locale/devtools/styleinspector.properties"));CssLogic.isContentStylesheet=function CssLogic_isContentStylesheet(aSheet)
...
CssLogic.getBackgroundImageUriFromProperty=function(aProperty,aSheetHref){let startToken="url(",start=aProperty.indexOf(startToken),end;if(start===-1){return null;}
aProperty=aProperty.substring(start+startToken.length).trim();let quote=aProperty.substring(0,1);if(quote==="'"||quote==='"'){end=aProperty.search(new RegExp(quote+"\\s*\\)"));start=1;}else{end=aProperty.indexOf(")");start=0;}
let uri=aProperty.substring(start,end).trim();if(aSheetHref){let IOService=Cc["@mozilla.org/network/io-service;1"].getService(Ci.nsIIOService);let sheetUri=IOService.newURI(aSheetHref,null,null);uri=sheetUri.resolve(uri);}
return uri;}
function positionInNodeList(element,nodeList){for(var i=0;i<nodeList.length;i++){if(element===nodeList[i]){return i;}}
return-1;}
CssLogic.findCssSelector=function CssLogic_findCssSelector(ele){var document=ele.ownerDocument;if(ele.id&&document.getElementById(ele.id)===ele){return'#'+ele.id;} 
...
index=positionInNodeList(ele,ele.parentNode.children)+1;selector=CssLogic_findCssSelector(ele.parentNode)+' > '+
tagName+':nth-child('+index+')';return selector;};function CssSheet(aCssLogic,aDomSheet,aIndex)
...
{if(!this._value&&this._cssLogic._computedStyle){try{this._value=this._cssLogic._computedStyle.getPropertyValue(this.property);}catch(ex){Services.console.logStringMessage('Error reading computed style for '+
...
{return this.selector+" -> "+this.value;},};XPCOMUtils.defineLazyGetter(this,"domUtils",function(){return Cc["@mozilla.org/inspector/dom-utils;1"].getService(Ci.inIDOMUtils);});