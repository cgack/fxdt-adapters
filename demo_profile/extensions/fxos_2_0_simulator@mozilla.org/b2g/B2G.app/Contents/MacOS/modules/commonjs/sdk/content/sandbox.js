...
let apiSandbox=sandbox(principals,{wantXrays:true,sameZoneAs:window});apiSandbox.console=console;
let content=sandbox(principals,{sandboxPrototype:proto,wantXrays:true,wantGlobalProperties:wantGlobalProperties,wantExportHelpers:true,sameZoneAs:window,metadata:{SDKContentScript:true,'inner-window-id':getInnerId(window)}});model.sandbox=content;
...
load(apiSandbox,CONTENT_WORKER_URL);let options='contentScriptOptions'in worker?JSON.stringify(worker.contentScriptOptions):undefined;





 let onEvent=onContentEvent.bind(null,this); let chromeAPI=createChromeAPI();let result=apiSandbox.ContentWorker.inject(content,chromeAPI,onEvent,options);
...
model.worker=null;},});exports.WorkerSandbox=WorkerSandbox;function importScripts(workerSandbox,...urls){let{worker,sandbox}=modelFor(workerSandbox);for(let i in urls){let contentScriptFile=urls[i];try{let uri=URL(contentScriptFile);if(uri.scheme==='resource')
