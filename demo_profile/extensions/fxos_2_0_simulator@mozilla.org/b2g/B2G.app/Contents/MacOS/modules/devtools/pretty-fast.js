...
if(ltt==","){return true;}
if(ltt==";"){return true;}
var ltk=lastToken.type.keyword;if(ltk!=null){if(ltk=="break"||ltk=="continue"){return token.type.type!=";";}
if(ltk!="debugger"&&ltk!="null"&&ltk!="true"&&ltk!="false"&&ltk!="this"&&ltk!="default"){return true;}}
if(ltt==")"&&(token.type.type!=")"&&token.type.type!="]"&&token.type.type!=";"&&token.type.type!=",")){return true;}}
...
function belongsOnStack(token){var ttt=token.type.type;var ttk=token.type.keyword;return ttt=="{"||ttt=="("||ttt=="["||ttt=="?"||ttk=="do"||ttk=="case"||ttk=="default";}
...
function incrementsIndent(token){return token.type.type=="{"||token.isArrayLiteral;}
...
if(decrementsIndent(ttt,stack)){indentLevel--;}
prependWhiteSpace(token,lastToken,addedNewline,write,options,indentLevel,stack);addToken(token,write,options);addedNewline=appendNewline(token,write,stack);if(shouldStackPop(token,stack)){stack.pop();}
...
for(var i=0,n=commentQueue.length;i<n;i++){var comment=commentQueue[i];addComment(write,indentLevel,options,comment.block,comment.text,comment.line,comment.column);}
