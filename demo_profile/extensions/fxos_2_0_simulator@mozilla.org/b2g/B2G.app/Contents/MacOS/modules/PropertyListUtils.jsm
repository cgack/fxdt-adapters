...
let swapped=this._swapForBigEndian(aByteOffset,aIntSize,aLength);if(aIntSize==2)
return new Uint16Array(swapped.buffer);if(aIntSize==4)
return new Uint32Array(swapped.buffer);if(aIntSize==8){let intsArray=[];let lo_hi_view=new Uint32Array(swapped.buffer);for(let i=0;i<lo_hi_view.length;i+=2){let[lo,hi]=[lo_hi_view[i],lo_hi_view[i+1]];let uint64=ctypes.UInt64.join(hi,lo);if(ctypes.UInt64.compare(uint64,this._JS_MAX_INT_UNSIGNED)==1){if(aBigIntAllowed===true)
intsArray.push(PropertyListUtils.wrapInt64(uint64.toString()));else
throw new Error("Integer too big to be read as float 64");}
else{intsArray.push(parseInt(uint64.toString(),10));}}
return intsArray;}
throw new Error("Unsupported size: "+aIntSize);},_readDataOffsetAndCount:function BPLR__readDataOffsetAndCount(aObjectOffset){
...
case this.OBJECT_TYPE_BITS.DATA:{let[offset,bytesCount]=this._readDataOffsetAndCount(objOffset);value=this._readUnsignedInts(offset,1,bytesCount);break;}
