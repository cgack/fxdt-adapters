...
return getTabForWindow(model.window);return null;},
 ...
 model.windowID=getInnerId(model.window);events.on("inner-window-destroyed",model.documentUnload);
model.window.addEventListener("pageshow",model.pageShow,true);model.window.addEventListener("pagehide",model.pageHide,true);model.contentWorker=WorkerSandbox(worker,model.window); model.inited=true;model.frozen=false; emit(worker,'attach',window);
...
model.inited=false;});destroy.define(Worker,function(worker,reason){detach(worker,reason);modelFor(worker).inited=true;
 off(worker);});function documentUnload({subject,data}){let model=modelFor(this);let innerWinID=subject.QueryInterface(Ci.nsISupportsPRUint64).data;if(innerWinID!=model.windowID)return false;detach(this);return true;}
