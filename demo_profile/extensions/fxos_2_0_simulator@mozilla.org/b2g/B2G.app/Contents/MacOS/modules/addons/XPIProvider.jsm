...
const STATIC_BLOCKLIST_PATTERNS=[{creator:"Mozilla Corp.",level:Ci.nsIBlocklistService.STATE_BLOCKED,blockID:"i162"},{creator:"Mozilla.org",level:Ci.nsIBlocklistService.STATE_BLOCKED,blockID:"i162"}];const BOOTSTRAP_REASONS={APP_STARTUP:1,APP_SHUTDOWN:2,ADDON_ENABLE:3,ADDON_DISABLE:4,ADDON_INSTALL:5,ADDON_UNINSTALL:6,ADDON_UPGRADE:7,ADDON_DOWNGRADE:8};const TYPES={extension:2,theme:4,locale:8,multipackage:32,dictionary:64,experiment:128,};const RESTARTLESS_TYPES=new Set(["dictionary","experiment","locale",]);
...
catch(e){logger.error("Failure "+(aCopy?"copying":"moving")+" "+aDirEntry.path+" to "+aTargetDirectory.path);throw e;}},move:function SIO_move(aFile,aTargetDirectory){try{this._installDirEntry(aFile,aTargetDirectory,false);}
...
catch(e){this.rollback();throw e;}},rollback:function SIO_rollback(){while(this._installedFiles.length>0){let move=this._installedFiles.pop();if(move.newFile.isDirectory()){let oldDir=move.oldFile.parent.clone();oldDir.append(move.oldFile.leafName);oldDir.create(Ci.nsIFile.DIRECTORY_TYPE,FileUtils.PERMS_DIRECTORY);}
...
return bestmatch;}
return null;}
function applyBlocklistChanges(aOldAddon,aNewAddon,aOldAppVersion,aOldPlatformVersion){ aNewAddon.userDisabled=aOldAddon.userDisabled;aNewAddon.softDisabled=aOldAddon.softDisabled;let bs=Cc["@mozilla.org/extensions/blocklist;1"].getService(Ci.nsIBlocklistService);let oldBlocklistState=bs.getAddonBlocklistState(createWrapper(aOldAddon),aOldAppVersion,aOldPlatformVersion);let newBlocklistState=bs.getAddonBlocklistState(createWrapper(aNewAddon));
 if(newBlocklistState==oldBlocklistState)
return;if(newBlocklistState==Ci.nsIBlocklistService.STATE_SOFTBLOCKED){if(aNewAddon.type!="theme"){
...
return true;if(aAddon.blocklistState==Ci.nsIBlocklistService.STATE_BLOCKED)
...
addon.strictCompatibility=!(addon.type in COMPATIBLE_BY_DEFAULT_TYPES)||getRDFProperty(ds,root,"strictCompatibility")=="true";if(addon.type=="extension"){addon.bootstrap=getRDFProperty(ds,root,"bootstrap")=="true";if(addon.optionsType&&addon.optionsType!=AddonManager.OPTIONS_TYPE_DIALOG&&addon.optionsType!=AddonManager.OPTIONS_TYPE_INLINE&&addon.optionsType!=AddonManager.OPTIONS_TYPE_TAB&&addon.optionsType!=AddonManager.OPTIONS_TYPE_INLINE_INFO){throw new Error("Install manifest specifies unknown type: "+addon.optionsType);}}
...
else{addon.userDisabled=false;addon.softDisabled=addon.blocklistState==Ci.nsIBlocklistService.STATE_SOFTBLOCKED;}
...
this.XPIProvider={ installLocations:null, installLocationsByName:null, installs:null, defaultSkin:"classic/1.0", currentSkin:null,
 ...
 allAppGlobal:true, enabledAddons:null, inactiveAddonIDs:[], runPhase:XPI_STARTING,
_mostRecentlyModifiedFile:{}, _telemetryDetails:{},_enabledExperiments:null,setTelemetry:function XPI_setTelemetry(aId,aName,aValue){if(!this._telemetryDetails[aId])
this._telemetryDetails[aId]={};this._telemetryDetails[aId][aName]=aValue;},_inProgress:new Set(),doing:function XPI_doing(aCancellable){this._inProgress.add(aCancellable);},done:function XPI_done(aCancellable){return this._inProgress.delete(aCancellable);},cancelAll:function XPI_cancelAll(){ while(this._inProgress.size>0){for(let c of this._inProgress){try{c.cancel();}
catch(e){logger.warn("Cancel failed",e);}
this._inProgress.delete(c);}}},_addURIMapping:function XPI__addURIMapping(aID,aFile){try{
let uri=this._resolveURIToFile(getURIForResourceInFile(aFile,"."));if(!uri){throw new Error("Cannot resolve");}
this._ensureURIMappings();this._uriMappings[aID]=uri.spec;}
catch(ex){logger.warn("Failed to add URI mapping",ex);}},_ensureURIMappings:function XPI__ensureURIMappings(){if(this._uriMappings){return;} 
this._uriMappings=Object.create(null); let enabled=Object.create(null);let enabledAddons=this.enabledAddons||"";for(let a of enabledAddons.split(",")){a=decodeURIComponent(a.split(":")[0]);enabled[a]=null;}
let cache=JSON.parse(Prefs.getCharPref(PREF_INSTALL_CACHE,"[]"));for(let loc of cache){for(let[id,val]in Iterator(loc.addons)){if(!(id in enabled)){continue;}
let file=new nsIFile(val.descriptor);let spec=Services.io.newFileURI(file).spec;this._uriMappings[id]=spec;}}},_resolveURIToFile:function XPI__resolveURIToFile(aURI){switch(aURI.scheme){case"jar":case"file":if(aURI instanceof Ci.nsIJARURI){return this._resolveURIToFile(aURI.JARFile);}
...
this.enabledAddons=Prefs.getCharPref(PREF_EM_ENABLED_ADDONS,"");delete this._uriMappings;if("nsICrashReporter"in Ci&&Services.appinfo instanceof Ci.nsICrashReporter){try{Services.appinfo.annotateCrashReport("Theme",this.currentSkin);}catch(e){}
...
reason=BOOTSTRAP_REASONS.ADDON_INSTALL;this.callBootstrapMethod(id,this.bootstrappedAddons[id].version,this.bootstrappedAddons[id].type,file,"startup",reason);}
...
Services.obs.addObserver({observe:function shutdownObserver(aSubject,aTopic,aData){for(let id in XPIProvider.bootstrappedAddons){let file=Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsIFile);file.persistentDescriptor=XPIProvider.bootstrappedAddons[id].descriptor;XPIProvider.callBootstrapMethod(id,XPIProvider.bootstrappedAddons[id].version,XPIProvider.bootstrappedAddons[id].type,file,"shutdown",BOOTSTRAP_REASONS.APP_SHUTDOWN);}
...
catch(e){logger.error("startup failed",e);AddonManagerPrivate.recordException("XPI","startup failed",e);}},shutdown:function XPI_shutdown(){logger.debug("shutdown"); this.cancelAll();this.bootstrappedAddons={};this.bootstrapScopes={};this.enabledAddons=null;this.allAppGlobal=true;this.inactiveAddonIDs=[];
...
Services.prefs.clearUserPref(PREF_DSS_SWITCHPENDING);},showUpgradeUI:function XPI_showUpgradeUI(){ Services.startup.interrupted=true;if(!Prefs.getBoolPref(PREF_SHOWN_SELECTION_UI,false)){var features="chrome,centerscreen,dialog,titlebar,modal";Services.ww.openWindow(null,URI_EXTENSION_SELECT_DIALOG,"",features,null);Services.prefs.setBoolPref(PREF_SHOWN_SELECTION_UI,true);}
else{var variant=Cc["@mozilla.org/variant;1"].createInstance(Ci.nsIWritableVariant);variant.setFromVariant(this.inactiveAddonIDs);var features="chrome,centerscreen,dialog,titlebar,modal";var ww=Cc["@mozilla.org/embedcomp/window-watcher;1"].getService(Ci.nsIWindowWatcher);ww.openWindow(null,URI_EXTENSION_UPDATE_DIALOG,"",features,variant);} 
Services.prefs.setBoolPref(PREF_PENDING_OPERATIONS,!XPIDatabase.writeAddonsList());},persistBootstrappedAddons:function XPI_persistBootstrappedAddons(){let filtered={};for(let id in this.bootstrappedAddons){let entry=this.bootstrappedAddons[id];if(entry.type=="experiment"){continue;}
filtered[id]=entry;}
...
seenFiles.push(jsonfile.leafName);existingAddonID=aManifests[aLocation.name][id].existingAddonID||id;var oldBootstrap=null;logger.debug("Processing install of "+id+" in "+aLocation.name);if(existingAddonID in this.bootstrappedAddons){try{var existingAddon=aLocation.getLocationForID(existingAddonID);if(this.bootstrappedAddons[existingAddonID].descriptor==existingAddon.persistentDescriptor){oldBootstrap=this.bootstrappedAddons[existingAddonID];
 let newVersion=aManifests[aLocation.name][id].version;let oldVersion=oldBootstrap.version;let uninstallReason=Services.vc.compare(oldVersion,newVersion)<0?BOOTSTRAP_REASONS.ADDON_UPGRADE:BOOTSTRAP_REASONS.ADDON_DOWNGRADE;this.callBootstrapMethod(existingAddonID,oldBootstrap.version,oldBootstrap.type,existingAddon,"uninstall",uninstallReason,{newVersion:newVersion});this.unloadBootstrapScope(existingAddonID);flushStartupCache();}}
catch(e){}}
...
catch(e){logger.warn("Add-on is invalid",e);XPIDatabase.removeAddonMetadata(aOldAddon);if(!aInstallLocation.locked)
...
if(aOldAddon.visible&&aOldAddon.active&&aOldAddon.bootstrap){XPIProvider.bootstrappedAddons[aOldAddon.id]={version:aOldAddon.version,type:aOldAddon.type,descriptor:aAddonState.descriptor};}
return changed;}
...
catch(e){logger.warn("Add-on is invalid",e);
...
newAddon.active=XPIDatabase.activeBundles.indexOf(aAddonState.descriptor)!=-1;
 if(!newAddon.active&&newAddon.visible&&!isAddonDisabled(newAddon)){ if(newAddon.blocklistState==Ci.nsIBlocklistService.STATE_SOFTBLOCKED)
...
return false;installReason=Services.vc.compare(oldBootstrap.version,newDBAddon.version)<0?BOOTSTRAP_REASONS.ADDON_UPGRADE:BOOTSTRAP_REASONS.ADDON_DOWNGRADE;let oldAddonFile=Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsIFile);oldAddonFile.persistentDescriptor=oldBootstrap.descriptor;XPIProvider.callBootstrapMethod(newDBAddon.id,oldBootstrap.version,oldBootstrap.type,oldAddonFile,"uninstall",installReason,{newVersion:newDBAddon.version});XPIProvider.unloadBootstrapScope(newDBAddon.id);
 if(newDBAddon.bootstrap)
flushStartupCache();}
if(!newDBAddon.bootstrap)
return true; let file=Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsIFile);file.persistentDescriptor=aAddonState.descriptor;XPIProvider.callBootstrapMethod(newDBAddon.id,newDBAddon.version,newDBAddon.type,file,"install",installReason,extraParams);if(!newDBAddon.active)
...
if(aOldAddon.id in addonStates){let addonState=addonStates[aOldAddon.id];delete addonStates[aOldAddon.id]; if(aOldAddon.visible&&!aOldAddon.active)
XPIProvider.inactiveAddonIDs.push(aOldAddon.id); let loc=aOldAddon.defaultLocale;if(loc){XPIProvider.setTelemetry(aOldAddon.id,"name",loc.name);XPIProvider.setTelemetry(aOldAddon.id,"creator",loc.creator);} 
if(aOldAddon.updateDate!=addonState.mtime){if(addonState.mtime<aOldAddon.updateDate){this.setTelemetry(aOldAddon.id,"olderFile",{name:this._mostRecentlyModifiedFile[aOldAddon.id],mtime:addonState.mtime,oldtime:aOldAddon.updateDate});}
...
results.push(aInstall.wrapper);});aCallback(results);},mapURIToAddonID:function XPI_mapURIToAddonID(aURI){this._ensureURIMappings();let resolved=this._resolveURIToFile(aURI);if(!resolved){return null;}
resolved=resolved.spec;for(let[id,spec]in Iterator(this._uriMappings)){if(resolved.startsWith(spec)){return id;}}
...
return false;
 if(Services.appinfo.inSafeMode)
return false;
 return this.disableRequiresRestart(aAddon);},loadBootstrapScope:function XPI_loadBootstrapScope(aId,aFile,aVersion,aType){ this.bootstrappedAddons[aId]={version:aVersion,type:aType,descriptor:aFile.persistentDescriptor};this.persistBootstrappedAddons();this.addAddonsToCrashReporter(); if(aType=="locale"){this.bootstrapScopes[aId]=null;return;}
logger.debug("Loading bootstrap scope from "+aFile.path);let principal=Cc["@mozilla.org/systemprincipal;1"].createInstance(Ci.nsIPrincipal);if(!aFile.exists()){this.bootstrapScopes[aId]=new Cu.Sandbox(principal,{sandboxName:aFile.path,wantGlobalProperties:["indexedDB"],metadata:{addonID:aId}});logger.error("Attempted to load bootstrap scope from missing directory "+aFile.path);return;}
let uri=getURIForResourceInFile(aFile,"bootstrap.js").spec;if(aType=="dictionary")
uri="resource://gre/modules/addons/SpellCheckDictionaryBootstrap.js"
this.bootstrapScopes[aId]=new Cu.Sandbox(principal,{sandboxName:uri,wantGlobalProperties:["indexedDB"],metadata:{addonID:aId,URI:uri}});let loader=Cc["@mozilla.org/moz/jssubscript-loader;1"].createInstance(Ci.mozIJSSubScriptLoader); this._addURIMapping(aId,aFile);try{for(let name in BOOTSTRAP_REASONS)
...
try{BrowserToolboxProcess.setAddonOptions(aId,{global:this.bootstrapScopes[aId]});}
catch(e){}},unloadBootstrapScope:function XPI_unloadBootstrapScope(aId){delete this.bootstrapScopes[aId];delete this.bootstrappedAddons[aId];this.persistBootstrappedAddons();this.addAddonsToCrashReporter();try{BrowserToolboxProcess.setAddonOptions(aId,{global:null});}
catch(e){}},callBootstrapMethod:function XPI_callBootstrapMethod(aId,aVersion,aType,aFile,aMethod,aReason,aExtraParams){ if(Services.appinfo.inSafeMode)
return;let timeStart=new Date();if(aMethod=="startup"){logger.debug("Registering manifest for "+aFile.path);Components.manager.addBootstrappedManifestLocation(aFile);}
try{ if(!(aId in this.bootstrapScopes))
this.loadBootstrapScope(aId,aFile,aVersion,aType); if(aType=="locale")
return;if(!(aMethod in this.bootstrapScopes[aId])){logger.warn("Add-on "+aId+" is missing bootstrap method "+aMethod);return;}
let params={id:aId,version:aVersion,installPath:aFile.clone(),resourceURI:getURIForResourceInFile(aFile,"")};if(aExtraParams){for(let key in aExtraParams){params[key]=aExtraParams[key];}}
logger.debug("Calling bootstrap method "+aMethod+" on "+aId+" version "+
aVersion);try{this.bootstrapScopes[aId][aMethod](params,aReason);}
catch(e){logger.warn("Exception running bootstrap method "+aMethod+" on "+aId,e);}}
...
this.setTelemetry(aId,aMethod+"_MS",new Date()-timeStart);}},updateAddonDisabledState:function XPI_updateAddonDisabledState(aAddon,aUserDisabled,aSoftDisabled){if(!(aAddon.inDatabase))
...
if(!needsRestart){XPIDatabase.updateAddonActive(aAddon,!isDisabled);if(isDisabled){if(aAddon.bootstrap){let file=aAddon._installLocation.getLocationForID(aAddon.id);this.callBootstrapMethod(aAddon.id,aAddon.version,aAddon.type,file,"shutdown",BOOTSTRAP_REASONS.ADDON_DISABLE);this.unloadBootstrapScope(aAddon.id);}
AddonManagerPrivate.callAddonListeners("onDisabled",wrapper);}
else{if(aAddon.bootstrap){let file=aAddon._installLocation.getLocationForID(aAddon.id);this.callBootstrapMethod(aAddon.id,aAddon.version,aAddon.type,file,"startup",BOOTSTRAP_REASONS.ADDON_ENABLE);}
...
throw new Error("Can only uninstall installed addons.");if(aAddon._installLocation.locked)
throw new Error("Cannot uninstall addons from locked install locations");if("_hasResourceCache"in aAddon)
...
if(aAddon.bootstrap){let file=aAddon._installLocation.getLocationForID(aAddon.id);XPIProvider.callBootstrapMethod(aAddon.id,aAddon.version,aAddon.type,file,"install",BOOTSTRAP_REASONS.ADDON_INSTALL);if(aAddon.active){XPIProvider.callBootstrapMethod(aAddon.id,aAddon.version,aAddon.type,file,"startup",BOOTSTRAP_REASONS.ADDON_INSTALL);}
...
if(!requiresRestart){if(aAddon.bootstrap){let file=aAddon._installLocation.getLocationForID(aAddon.id);if(aAddon.active){this.callBootstrapMethod(aAddon.id,aAddon.version,aAddon.type,file,"shutdown",BOOTSTRAP_REASONS.ADDON_UNINSTALL);}
this.callBootstrapMethod(aAddon.id,aAddon.version,aAddon.type,file,"uninstall",BOOTSTRAP_REASONS.ADDON_UNINSTALL);this.unloadBootstrapScope(aAddon.id);flushStartupCache();}
...
reason=BOOTSTRAP_REASONS.ADDON_DOWNGRADE;if(this.existingAddon.bootstrap){let file=this.existingAddon._installLocation.getLocationForID(this.existingAddon.id);if(this.existingAddon.active){XPIProvider.callBootstrapMethod(this.existingAddon.id,this.existingAddon.version,this.existingAddon.type,file,"shutdown",reason,{newVersion:this.addon.version});}
XPIProvider.callBootstrapMethod(this.existingAddon.id,this.existingAddon.version,this.existingAddon.type,file,"uninstall",reason,{newVersion:this.addon.version});XPIProvider.unloadBootstrapScope(this.existingAddon.id);flushStartupCache();}
...
if(this.addon.bootstrap){XPIProvider.callBootstrapMethod(this.addon.id,this.addon.version,this.addon.type,file,"install",reason,extraParams);}
AddonManagerPrivate.callAddonListeners("onInstalled",createWrapper(this.addon));logger.debug("Install of "+this.sourceURI.spec+" completed.");this.state=AddonManager.STATE_INSTALLED;AddonManagerPrivate.callInstallListeners("onInstallEnded",this.listeners,this.wrapper,createWrapper(this.addon));if(this.addon.bootstrap){if(this.addon.active){XPIProvider.callBootstrapMethod(this.addon.id,this.addon.version,this.addon.type,file,"startup",reason,extraParams);}
else{
 XPIProvider.unloadBootstrapScope(this.addon.id);}}
XPIProvider.setTelemetry(this.addon.id,"unpacked",installedUnpacked);XPIProvider.setTelemetry(this.addon.id,"location",this.installLocation.name);XPIProvider.setTelemetry(this.addon.id,"scan_MS",scanTime);XPIProvider.setTelemetry(this.addon.id,"scan_items",scanItems);let loc=this.addon.defaultLocale;if(loc){XPIProvider.setTelemetry(this.addon.id,"name",loc.name);XPIProvider.setTelemetry(this.addon.id,"creator",loc.creator);}}}).bind(this)).then(null,(e)=>{logger.warn("Failed to install "+this.file.path+" from "+this.sourceURI.spec,e);if(stagedAddon.exists())
...
return staticItem.level;let bs=Cc["@mozilla.org/extensions/blocklist;1"].getService(Ci.nsIBlocklistService);return bs.getAddonBlocklistState(createWrapper(this));},get blocklistURL(){let staticItem=findMatchingStaticBlocklistItem(this);if(staticItem){let url=Services.urlFormatter.formatURLPref("extensions.blocklist.itemURL");return url.replace(/%blockID%/g,staticItem.blockID);}
let bs=Cc["@mozilla.org/extensions/blocklist;1"].getService(Ci.nsIBlocklistService);return bs.getAddonBlocklistURL(createWrapper(this));},applyCompatibilityUpdate:function AddonInternal_applyCompatibilityUpdate(aUpdate,aSyncCompatibility){this.targetApplications.forEach(function(aTargetApp){aUpdate.targetApplications.forEach(function(aUpdateTarget){if(aTargetApp.id==aUpdateTarget.id&&(aSyncCompatibility||Services.vc.compare(aTargetApp.maxVersion,aUpdateTarget.maxVersion)<0)){aTargetApp.minVersion=aUpdateTarget.minVersion;aTargetApp.maxVersion=aUpdateTarget.maxVersion;}});});this.appDisabled=!isUsableAddon(this);},toJSON:function AddonInternal_toJSON(aKey){let obj={};for(let prop in this){ if(prop.substring(0,1)=="_")
...
return;this[aProp]=aObj[aProp];},this); this.appDisabled=!isUsableAddon(this);}};function createWrapper(aAddon){if(!aAddon)
...
["id","syncGUID","version","type","isCompatible","isPlatformCompatible","providesUpdatesSecurely","blocklistState","blocklistURL","appDisabled","softDisabled","skinnable","size","foreignInstall","hasBinaryComponents","strictCompatibility","compatibilityOverrides","updateURL"].forEach(function(aProp){this.__defineGetter__(aProp,function AddonWrapper_propertyGetter()aAddon[aProp]);},this);["fullDescription","developerComments","eula","supportURL","contributionURL","contributionAmount","averageRating","reviewCount","reviewURL","totalDownloads","weeklyDownloads","dailyUsers","repositoryStatus"].forEach(function(aProp){this.__defineGetter__(aProp,function AddonWrapper_repoPropertyGetter(){if(aAddon._repositoryAddon)
...
pending|=AddonManager.PENDING_ENABLE;if(aAddon.pendingUpgrade)
...
ops|=AddonManager.OP_NEEDS_RESTART_DISABLE;return ops;});this.__defineGetter__("isDebuggable",function AddonWrapper_isDebuggable(){return this.isActive&&aAddon.bootstrap;});this.__defineGetter__("permissions",function AddonWrapper_permisionsGetter(){let permissions=0; if(!(aAddon.inDatabase))
return permissions;

if(aAddon.type=="experiment"){return AddonManager.PERM_CAN_UNINSTALL;}
if(!aAddon.appDisabled){if(this.userDisabled){permissions|=AddonManager.PERM_CAN_ENABLE;}
else if(aAddon.type!="theme"){permissions|=AddonManager.PERM_CAN_DISABLE;}}
 
if(!aAddon._installLocation.locked&&!aAddon.pendingUninstall){ if(!aAddon._installLocation.isLinkedAddon(aAddon.id)){permissions|=AddonManager.PERM_CAN_UPGRADE;}
permissions|=AddonManager.PERM_CAN_UNINSTALL;}
return permissions;});this.__defineGetter__("isActive",function AddonWrapper_isActiveGetter(){if(Services.appinfo.inSafeMode)
...
this._IDToFileMap[id]=entry;this._FileToIDMap[entry.path]=id;}},get name(){return this._name;},get scope(){return this._scope;},get addonLocations(){let locations=[];for(let id in this._IDToFileMap){locations.push(this._IDToFileMap[id].clone());}
...
transaction.move(file,trashDir);file=self._directory.clone();file.append(aId+".xpi");if(file.exists()){flushJarCache(file);transaction.move(file,trashDir);}}
 
try{moveOldAddon(aId);if(aExistingAddonID&&aExistingAddonID!=aId)
moveOldAddon(aExistingAddonID);if(aCopy){transaction.copy(aSource,this._directory);}
else{if(aSource.isFile())
flushJarCache(aSource);transaction.move(aSource,this._directory);}}
finally{
...
this._FileToIDMap[newFile.path]=aId;this._IDToFileMap[aId]=newFile;if(aExistingAddonID&&aExistingAddonID!=aId&&aExistingAddonID in this._IDToFileMap){delete this._FileToIDMap[this._IDToFileMap[aExistingAddonID]];delete this._IDToFileMap[aExistingAddonID];}
...
let transaction=new SafeInstallOperation();try{transaction.move(file,trashDir);}
finally{
 try{recursiveRemove(trashDir);}
