...
try{LoginManagerContent._onFormSubmit(formElement);}catch(e){log("Caught error in onFormSubmit:",e);}
...
this.__formFillService=Cc["@mozilla.org/satchel/form-fill-controller;1"].getService(Ci.nsIFormFillController);return this.__formFillService;},onFormPassword:function(event){if(!event.isTrusted)
return;if(!gEnabled)
return;let form=event.target;let doc=form.ownerDocument;log("onFormPassword for",doc.documentURI);let formOrigin=LoginUtils._getPasswordOrigin(doc.documentURI);if(!Services.logins.countLogins(formOrigin,"",null))
return;
...
return;self.onFormPassword(event);},handleEvent:function(event){}};


Services.obs.addObserver(observer,"passwordmgr-crypto-login",true);Services.obs.addObserver(observer,"passwordmgr-crypto-loginCanceled",true);form.addEventListener("mozCleverClosureHack",observer);return;}
let autofillForm=gAutofillForms&&!PrivateBrowsingUtils.isWindowPrivate(doc.defaultView);this._fillForm(form,autofillForm,false,false,null);},onUsernameInput:function(event){if(!event.isTrusted)
return;if(!gEnabled)
...
return;this._fillForm(acForm,true,true,true,null);}else{}},_getPasswordFields:function(form,skipEmptyFields){var pwFields=[];for(var i=0;i<form.elements.length;i++){var element=form.elements[i];if(!(element instanceof Ci.nsIDOMHTMLInputElement)||element.type!="password")
...
return true;return false;},_onFormSubmit:function(form){function getPrompter(aWindow){var prompterSvc=Cc["@mozilla.org/login-manager/prompter;1"].createInstance(Ci.nsILoginManagerPrompter);prompterSvc.init(aWindow);return prompterSvc;}
var doc=form.ownerDocument;var win=doc.defaultView;if(PrivateBrowsingUtils.isWindowPrivate(win)){log("(form submission ignored in private browsing mode)");return;}
if(!gEnabled)
...
var prompter=getPrompter(win);if(logins.length==1){var oldLogin=logins[0];formLogin.username=oldLogin.username;formLogin.usernameField=oldLogin.usernameField;prompter.promptToChangePassword(oldLogin,formLogin);}else{prompter.promptToChangePasswordWithUsernames(logins,logins.length,formLogin);}
return;}
...
if(existingLogin){log("Found an existing login matching this form submission");if(existingLogin.password!=formLogin.password){log("...passwords differ, prompting to change.");prompter=getPrompter(win);prompter.promptToChangePassword(existingLogin,formLogin);}else{var propBag=Cc["@mozilla.org/hash-property-bag;1"].createInstance(Ci.nsIWritablePropertyBag);propBag.setProperty("timeLastUsed",Date.now());propBag.setProperty("timesUsedIncrement",1);Services.logins.modifyLogin(existingLogin,propBag);}
return;}
prompter=getPrompter(win);prompter.promptToSavePassword(formLogin);},_fillForm:function(form,autofillForm,ignoreAutocomplete,clobberPassword,foundLogins){
...
if(foundLogins==null){var formOrigin=LoginUtils._getPasswordOrigin(form.ownerDocument.documentURI);var actionOrigin=LoginUtils._getActionOrigin(form);foundLogins=Services.logins.findLogins({},formOrigin,actionOrigin,null);log("found",foundLogins.length,"matching logins.");}else{log("reusing logins from last form.");}
...
l.username.toLowerCase()==username);if(matchingLogins.length){selectedLogin=matchingLogins[0];}else{didntFillReason="existingUsername";log("Password not filled. None of the stored logins match the username already present.");}}else if(logins.length==1){selectedLogin=logins[0];}else{
...
var didFillForm=false;if(selectedLogin&&autofillForm&&!isFormDisabled){
if(usernameField&&!(usernameField.disabled||usernameField.readOnly))
usernameField.value=selectedLogin.username;passwordField.value=selectedLogin.password;didFillForm=true;}else if(selectedLogin&&!autofillForm){
...
uriString=form.baseURI; return this._getPasswordOrigin(uriString,true);},};