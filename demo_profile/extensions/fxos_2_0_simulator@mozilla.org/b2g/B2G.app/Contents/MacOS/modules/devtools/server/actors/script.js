"use strict";const Debugger=require("Debugger");const Services=require("Services");const{Cc,Ci,Cu,components}=require("chrome");const{ActorPool}=require("devtools/server/actors/common");const{DebuggerServer}=require("devtools/server/main");const DevToolsUtils=require("devtools/toolkit/DevToolsUtils");const{dbg_assert,dumpn,update}=DevToolsUtils;const{SourceMapConsumer,SourceMapGenerator}=require("source-map");const{all,defer,resolve}=promise;Cu.import("resource://gre/modules/NetUtil.jsm");let B2G_ID="{3c2e2abc-06d4-11e1-ac3b-374f68613e61}";let TYPED_ARRAY_CLASSES=["Uint8Array","Uint8ClampedArray","Uint16Array","Uint32Array","Int8Array","Int16Array","Int32Array","Float32Array","Float64Array"];let OBJECT_PREVIEW_MAX_ITEMS=10;let addonManager=null;function mapURIToAddonID(uri,id){if(Services.appinfo.ID==B2G_ID){return false;}
if(!addonManager){addonManager=Cc["@mozilla.org/addons/integration;1"].getService(Ci.amIAddonManager);}
try{return addonManager.mapURIToAddonID(uri,id);}
catch(e){DevToolsUtils.reportException("mapURIToAddonID",e);return false;}}
function BreakpointStore(){this._size=0;
...
BreakpointStore.prototype={_size:null,get size(){return this._size;},addBreakpoint:function(aBreakpoint){let{url,line,column}=aBreakpoint;let updating=false;if(column!=null){if(!this._breakpoints[url]){this._breakpoints[url]=[];}
if(!this._breakpoints[url][line]){this._breakpoints[url][line]=[];}
if(this._breakpoints[url][line][column]){updating=true;}
this._breakpoints[url][line][column]=aBreakpoint;}else{if(!this._wholeLineBreakpoints[url]){this._wholeLineBreakpoints[url]=[];}
if(this._wholeLineBreakpoints[url][line]){updating=true;}
this._wholeLineBreakpoints[url][line]=aBreakpoint;}
if(!updating){this._size++;}},removeBreakpoint:function({url,line,column}){if(column!=null){if(this._breakpoints[url]){if(this._breakpoints[url][line]){if(this._breakpoints[url][line][column]){delete this._breakpoints[url][line][column];this._size--;
...
if(aColumn!=null){if(this._breakpoints[aUrl][aLine][aColumn]){yield aColumn;}}else{for(let column in this._breakpoints[aUrl][aLine]){yield column;}}},};exports.BreakpointStore=BreakpointStore;function EventLoopStack({inspector,thread,connection,hooks}){this._inspector=inspector;this._hooks=hooks;this._thread=thread;this._connection=connection;}
EventLoopStack.prototype={get size(){return this._inspector.eventLoopNestLevel;},get lastPausedUrl(){let url=null;if(this.size>0){try{url=this._inspector.lastNestRequestor.url}catch(e){
dumpn(e);}}
return url;},get lastConnection(){return this._inspector.lastNestRequestor._connection;},push:function(){return new EventLoop({inspector:this._inspector,thread:this._thread,connection:this._connection,hooks:this._hooks});}};function EventLoop({inspector,thread,connection,hooks}){this._inspector=inspector;this._thread=thread;this._hooks=hooks;this._connection=connection;this.enter=this.enter.bind(this);this.resolve=this.resolve.bind(this);}
EventLoop.prototype={entered:false,resolved:false,get url(){return this._hooks.url;},enter:function(){let nestData=this._hooks.preNest?this._hooks.preNest():null;this.entered=true;this._inspector.enterNestedEventLoop(this);if(this._inspector.eventLoopNestLevel>0){const{resolved}=this._inspector.lastNestRequestor;if(resolved){this._inspector.exitNestedEventLoop();}}
...
this.resolved=true;if(this===this._inspector.lastNestRequestor){this._inspector.exitNestedEventLoop();return true;}
return false;},};function ThreadActor(aHooks,aGlobal)
{this._state="detached";this._frameActors=[];this._hooks=aHooks;this.global=aGlobal;this._hiddenBreakpoints=new Map();this.findGlobals=this.globalManager.findGlobals.bind(this);this.onNewGlobal=this.globalManager.onNewGlobal.bind(this);this.onNewSource=this.onNewSource.bind(this);this._allEventsListener=this._allEventsListener.bind(this);this._options={useSourceMaps:false};this._gripDepth=0;}
ThreadActor.breakpointStore=new BreakpointStore();ThreadActor.prototype={_gripDepth:null,actorPrefix:"context",get state(){return this._state;},get attached()this.state=="attached"||this.state=="running"||this.state=="paused",get breakpointStore(){return ThreadActor.breakpointStore;},get threadLifetimePool(){if(!this._threadLifetimePool){this._threadLifetimePool=new ActorPool(this.conn);this.conn.addActorPool(this._threadLifetimePool);this._threadLifetimePool.objectActors=new WeakMap();}
return this._threadLifetimePool;},get sources(){if(!this._sources){this._sources=new ThreadSources(this,this._options.useSourceMaps,this._allowSource,this.onNewSource);}
...
const eventLoop=this._nestedEventLoops.push();this._threadPauseEventLoops.push(eventLoop);eventLoop.enter();},_popThreadPause:function(){const eventLoop=this._threadPauseEventLoops.pop();dbg_assert(eventLoop,"Should have an event loop.");eventLoop.resolve();},clearDebuggees:function(){if(this.dbg){this.dbg.removeAllDebuggees();}
this._sources=null;},addDebuggee:function(aGlobal){let globalDebugObject;try{globalDebugObject=this.dbg.addDebuggee(aGlobal);}catch(e){dumpn("Ignoring request to add the debugger's compartment as a debuggee");}
return globalDebugObject;},_initDebugger:function(){this.dbg=new Debugger();this.dbg.uncaughtExceptionHook=this.uncaughtExceptionHook.bind(this);this.dbg.onDebuggerStatement=this.onDebuggerStatement.bind(this);this.dbg.onNewScript=this.onNewScript.bind(this);this.dbg.onNewGlobalObject=this.globalManager.onNewGlobal.bind(this);this.dbg.enabled=this._state!="detached";},removeDebugee:function(aGlobal){try{this.dbg.removeDebuggee(aGlobal);}catch(ex){}},_addDebuggees:function(aWindow){let globalDebugObject=this.addDebuggee(aWindow);let frames=aWindow.frames;if(frames){for(let i=0;i<frames.length;i++){this._addDebuggees(frames[i]);}}
return globalDebugObject;},globalManager:{findGlobals:function(){const{gDevToolsExtensions:{getContentGlobals}}=Cu.import("resource://gre/modules/devtools/DevToolsExtensions.jsm",{});this.globalDebugObject=this._addDebuggees(this.global); try{getContentGlobals({'inner-window-id':getInnerId(this.global)}).forEach(this.addDebuggee.bind(this));}
catch(e){}},onNewGlobal:function(aGlobal){let useGlobal=(aGlobal.hostAnnotations&&aGlobal.hostAnnotations.type=="document"&&aGlobal.hostAnnotations.element===this.global); if(!useGlobal){let metadata={};let id="";try{id=getInnerId(this.global);metadata=Cu.getSandboxMetadata(aGlobal.unsafeDereference());}
catch(e){}
useGlobal=(metadata['inner-window-id']&&metadata['inner-window-id']==id);}
if(useGlobal){this.addDebuggee(aGlobal);this.conn.send({from:this.actorID,type:"newGlobal",hostAnnotations:aGlobal.hostAnnotations});}}},disconnect:function(){dumpn("in ThreadActor.prototype.disconnect");if(this._state=="paused"){this.onResume();}
...
if(!this.dbg){return;}
this.dbg.enabled=false;this.dbg=null;},exit:function(){this.disconnect();this._state="exited";}, onAttach:function(aRequest){if(this.state==="exited"){return{type:"exited"};}
if(this.state!=="detached"){return{error:"wrongState",message:"Current state is "+this.state};}
this._state="attached";update(this._options,aRequest.options||{});this._nestedEventLoops=new EventLoopStack({inspector:DebuggerServer.xpcInspector,hooks:this._hooks,connection:this.conn,thread:this});if(!this.dbg){this._initDebugger();}
this.findGlobals();this.dbg.enabled=true;try{let packet=this._paused();if(!packet){return{error:"notAttached"};}
...
return undefined;},_forceCompletion:function(aRequest){
...
return aPacket;});};},_makeOnStep:function({thread,pauseAndRespond,startFrame,startLocation}){return function(){const generatedLocation=getFrameLocation(this);const newLocation=thread.synchronize(thread.sources.getOriginalLocation(generatedLocation));
...
return undefined;};},_makeSteppingHooks:function(aStartLocation){


const steppingHookState={pauseAndRespond:(aFrame,onPacket=(k)=>k)=>{this._pauseAndRespond(aFrame,{type:"resumeLimit"},onPacket);},createValueGrip:this.createValueGrip.bind(this),thread:this,startFrame:this.youngestFrame,startLocation:aStartLocation};return{onEnterFrame:this._makeOnEnterFrame(steppingHookState),onPop:this._makeOnPop(steppingHookState),onStep:this._makeOnStep(steppingHookState)};},_handleResumeLimit:function(aRequest){let steppingType=aRequest.resumeLimit.type;if(["step","next","finish"].indexOf(steppingType)==-1){return reject({error:"badParameterType",message:"Unknown resumeLimit type"});}
const generatedLocation=getFrameLocation(this.youngestFrame);return this.sources.getOriginalLocation(generatedLocation).then(originalLocation=>{const{onEnterFrame,onPop,onStep}=this._makeSteppingHooks(originalLocation);
let stepFrame=this._getNextStepFrame(this.youngestFrame);if(stepFrame){switch(steppingType){case"step":this.dbg.onEnterFrame=onEnterFrame;case"next":if(stepFrame.script){stepFrame.onStep=onStep;}
stepFrame.onPop=onPop;break;case"finish":stepFrame.onPop=onPop;}}
return true;});},_clearSteppingHooks:function(aFrame){while(aFrame){aFrame.onStep=undefined;aFrame.onPop=undefined;aFrame=aFrame.older;}},_maybeListenToEvents:function(aRequest){let events=aRequest.pauseOnDOMEvents;if(this.global&&events&&(events=="*"||(Array.isArray(events)&&events.length))){this._pauseOnDOMEvents=events;let els=Cc["@mozilla.org/eventlistenerservice;1"].getService(Ci.nsIEventListenerService);els.addListenerForAllEvents(this.global,this._allEventsListener,true);}},onResume:function(aRequest){if(this._state!=="paused"){return{error:"wrongState",message:"Can't resume when debuggee isn't paused. Current state is '"
+this._state+"'"};}

if(this._nestedEventLoops.size&&this._nestedEventLoops.lastPausedUrl&&(this._nestedEventLoops.lastPausedUrl!==this._hooks.url||this._nestedEventLoops.lastConnection!==this.conn)){return{error:"wrongOrder",message:"trying to resume in the wrong order.",lastPausedUrl:this._nestedEventLoops.lastPausedUrl};}
...
continue;let l=Object.create(null);l.type=handler.type;let listener=handler.listenerObject;l.script=this.globalDebugObject.makeDebuggeeValue(listener).script;
if(!l.script)
...
let scripts=this.dbg.findScripts(aLocation);if(scripts.length==0){return{error:"noScript",message:"Requested setting a breakpoint on "
+aLocation.url+":"+aLocation.line
+(aLocation.column!=null?":"+aLocation.column:"")
+" but there is no Debugger.Script at that location",actor:actor.actorID};} 
...
actor.addScript(script,this);if(!actualLocation){actualLocation={url:aLocation.url,line:line,column:0};}
found=true;break;}}}
...
return this.objectGrip(aValue,aPool);default:dbg_assert(false,"Failed to provide a grip for: "+aValue);return null;}},createProtocolCompletionValue:function(aCompletion){let protoValue={};if("return"in aCompletion){protoValue.return=this.createValueGrip(aCompletion.return);}else if("yield"in aCompletion){protoValue.return=this.createValueGrip(aCompletion.yield);}else if("throw"in aCompletion){protoValue.throw=this.createValueGrip(aCompletion.throw);}else{protoValue.terminated=true;}
...
{this.obj=aObj;this.threadActor=aThreadActor;}
ObjectActor.prototype={actorPrefix:"obj",grip:function(){this.threadActor._gripDepth++;let g={"type":"object","class":this.obj.class,"actor":this.actorID,"extensible":this.obj.isExtensible(),"frozen":this.obj.isFrozen(),"sealed":this.obj.isSealed()};if(this.obj.class!="DeadObject"){let raw=Cu.unwaiveXrays(this.obj.unsafeDereference());if(!DevToolsUtils.isSafeJSObject(raw)){raw=null;}
...
let raw=obj.unsafeDereference();let items=aGrip.preview.items=[];for(let[i,value]of Array.prototype.entries.call(raw)){if(Object.hasOwnProperty.call(raw,i)){value=makeDebuggeeValueIfNeeded(obj,value);items.push(threadActor.createValueGrip(value));}else{items.push(null);}
...
let raw=obj.unsafeDereference();let items=aGrip.preview.items=[];for(let item of Set.prototype.values.call(raw)){item=makeDebuggeeValueIfNeeded(obj,item);items.push(threadActor.createValueGrip(item));if(items.length==OBJECT_PREVIEW_MAX_ITEMS){break;}}
...
aGrip.preview={kind:"ArrayLike",length:length,};if(threadActor._gripDepth>1){return true;}
let raw=obj.unsafeDereference();let global=Cu.getGlobalForObject(DebuggerServer);let classProto=global[obj.class].prototype;let safeView=classProto.subarray.call(raw,0,OBJECT_PREVIEW_MAX_ITEMS);let items=aGrip.preview.items=[];for(let i=0;i<safeView.length;i++){items.push(safeView[i]);}
return true;},function Error({obj,threadActor},aGrip){switch(obj.class){case"Error":case"EvalError":case"RangeError":case"ReferenceError":case"SyntaxError":case"TypeError":case"URIError":let name=DevToolsUtils.getProperty(obj,"name");let msg=DevToolsUtils.getProperty(obj,"message");let stack=DevToolsUtils.getProperty(obj,"stack");let fileName=DevToolsUtils.getProperty(obj,"fileName");let lineNumber=DevToolsUtils.getProperty(obj,"lineNumber");let columnNumber=DevToolsUtils.getProperty(obj,"columnNumber");aGrip.preview={kind:"Error",name:threadActor.createValueGrip(name),message:threadActor.createValueGrip(msg),stack:threadActor.createValueGrip(stack),fileName:threadActor.createValueGrip(fileName),lineNumber:threadActor.createValueGrip(lineNumber),columnNumber:threadActor.createValueGrip(columnNumber),};return true;default:return false;}},function CSSMediaRule({obj,threadActor},aGrip,aRawObj){if(!aRawObj||!(aRawObj instanceof Ci.nsIDOMCSSMediaRule)){return false;}
aGrip.preview={kind:"ObjectWithText",text:threadActor.createValueGrip(aRawObj.conditionText),};return true;},function CSSStyleRule({obj,threadActor},aGrip,aRawObj){if(!aRawObj||!(aRawObj instanceof Ci.nsIDOMCSSStyleRule)){return false;}
aGrip.preview={kind:"ObjectWithText",text:threadActor.createValueGrip(aRawObj.selectorText),};return true;},function ObjectWithURL({obj,threadActor},aGrip,aRawObj){if(!aRawObj||!(aRawObj instanceof Ci.nsIDOMCSSImportRule||aRawObj instanceof Ci.nsIDOMCSSStyleSheet||aRawObj instanceof Ci.nsIDOMLocation||aRawObj instanceof Ci.nsIDOMWindow)){return false;}
...
aGrip.preview={kind:"ObjectWithURL",url:threadActor.createValueGrip(url),};return true;},function ArrayLike({obj,threadActor},aGrip,aRawObj){if(!aRawObj||obj.class!="DOMStringList"&&obj.class!="DOMTokenList"&&!(aRawObj instanceof Ci.nsIDOMMozNamedAttrMap||aRawObj instanceof Ci.nsIDOMCSSRuleList||aRawObj instanceof Ci.nsIDOMCSSValueList||aRawObj instanceof Ci.nsIDOMFileList||aRawObj instanceof Ci.nsIDOMFontFaceList||aRawObj instanceof Ci.nsIDOMMediaList||aRawObj instanceof Ci.nsIDOMNodeList||aRawObj instanceof Ci.nsIDOMStyleSheetList)){return false;}
...
return true;}, function CSSStyleDeclaration({obj,threadActor},aGrip,aRawObj){if(!aRawObj||!(aRawObj instanceof Ci.nsIDOMCSSStyleDeclaration)){return false;}
...
return true;},function DOMNode({obj,threadActor},aGrip,aRawObj){if(obj.class=="Object"||!aRawObj||!(aRawObj instanceof Ci.nsIDOMNode)){return false;}
...
return true;}, function DOMEvent({obj,threadActor},aGrip,aRawObj){if(!aRawObj||!(aRawObj instanceof Ci.nsIDOMEvent)){return false;}
...
return true;}, function DOMException({obj,threadActor},aGrip,aRawObj){if(!aRawObj||!(aRawObj instanceof Ci.nsIDOMDOMException)){return false;}
...
let desc={configurable:false,writable:true,enumerable:true};try{desc.value=this.obj.getVariable(name);}catch(e){
if(name!="arguments"){throw e;}}
let descForm={enumerable:true,configurable:desc.configurable};if("value"in desc){descForm.value=this.threadActor.createValueGrip(desc.value);descForm.writable=desc.writable;}else{descForm.get=this.threadActor.createValueGrip(desc.get);descForm.set=this.threadActor.createValueGrip(desc.set);}
...
return output;};Object.defineProperty(Debugger.Frame.prototype,"line",{configurable:true,get:function(){if(this.script){return this.script.getOffsetLine(this.offset);}else{return null;}}});function ChromeDebuggerActor(aConnection,aHooks)
{ThreadActor.call(this,aHooks);}
ChromeDebuggerActor.prototype=Object.create(ThreadActor.prototype);update(ChromeDebuggerActor.prototype,{constructor:ChromeDebuggerActor,actorPrefix:"chromeDebugger",_allowSource:function(aSourceURL)!!aSourceURL,globalManager:{findGlobals:function(){this.dbg.addAllGlobalsAsDebuggees();},onNewGlobal:function(aGlobal){this.addDebuggee(aGlobal);this.conn.send({from:this.actorID,type:"newGlobal",hostAnnotations:aGlobal.hostAnnotations});}}});exports.ChromeDebuggerActor=ChromeDebuggerActor;function AddonThreadActor(aConnect,aHooks,aAddonID){this.addonID=aAddonID;ThreadActor.call(this,aHooks);}
AddonThreadActor.prototype=Object.create(ThreadActor.prototype);update(AddonThreadActor.prototype,{constructor:AddonThreadActor,actorPrefix:"addonThread",onAttach:function(aRequest){if(!this.attached){Services.obs.addObserver(this,"chrome-document-global-created",false);Services.obs.addObserver(this,"content-document-global-created",false);}
return ThreadActor.prototype.onAttach.call(this,aRequest);},disconnect:function(){if(this.attached){Services.obs.removeObserver(this,"content-document-global-created");Services.obs.removeObserver(this,"chrome-document-global-created");}
return ThreadActor.prototype.disconnect.call(this);},observe:function(aSubject,aTopic,aData){let id={};if(mapURIToAddonID(aSubject.location,id)&&id.value===this.addonID){this.dbg.addDebuggee(aSubject.defaultView);}},_allowSource:function(aSourceURL){ if(!aSourceURL){return false;} 
if(aSourceURL=="resource://gre/modules/addons/XPIProvider.jsm"){return false;}
return true;},globalManager:{findGlobals:function ADA_findGlobals(){for(let global of this.dbg.findAllGlobals()){if(this._checkGlobal(global)){this.dbg.addDebuggee(global);}}},onNewGlobal:function ADA_onNewGlobal(aGlobal){if(this._checkGlobal(aGlobal)){this.addDebuggee(aGlobal);this.conn.send({from:this.actorID,type:"newGlobal",hostAnnotations:aGlobal.hostAnnotations});}}},_checkGlobal:function ADA_checkGlobal(aGlobal){let obj=null;try{obj=aGlobal.unsafeDereference();}
catch(e){
 return false;}
try{let metadata=Cu.getSandboxMetadata(obj);if(metadata){return metadata.addonID===this.addonID;}}catch(e){}
if(obj instanceof Ci.nsIDOMWindow){let id={};if(mapURIToAddonID(obj.document.documentURIObject,id)){return id.value===this.addonID;}
return false;}
 
let uridescriptor=aGlobal.getOwnPropertyDescriptor("__URI__");if(uridescriptor&&"value"in uridescriptor&&uridescriptor.value){let uri;try{uri=Services.io.newURI(uridescriptor.value,null,null);}
catch(e){DevToolsUtils.reportException("AddonThreadActor.prototype._checkGlobal",new Error("Invalid URI: "+uridescriptor.value));return false;}
let id={};if(mapURIToAddonID(uri,id)){return id.value===this.addonID;}}
return false;}});AddonThreadActor.prototype.requestTypes=Object.create(ThreadActor.prototype.requestTypes);update(AddonThreadActor.prototype.requestTypes,{"attach":AddonThreadActor.prototype.onAttach});exports.AddonThreadActor=AddonThreadActor;function ThreadSources(aThreadActor,aUseSourceMaps,aAllowPredicate,aOnNewSource){this._thread=aThreadActor;this._useSourceMaps=aUseSourceMaps;this._allow=aAllowPredicate;this._onNewSource=aOnNewSource; this._sourceMapsByGeneratedSource=Object.create(null); this._sourceMapsByOriginalSource=Object.create(null); this._sourceActors=Object.create(null); this._generatedUrlsByOriginalUrl=Object.create(null);}
ThreadSources._blackBoxedSources=new Set(["self-hosted"]);ThreadSources._prettyPrintedSources=new Map();ThreadSources.prototype={source:function({url,sourceMap,generatedSource,text,contentType}){if(!this._allow(url)){return null;}
...
return actor;},_sourceForScript:function(aScript){const spec={url:aScript.url};



if(aScript.url){try{const url=Services.io.newURI(aScript.url,null,null).QueryInterface(Ci.nsIURL);if(url.fileExtension==="js"){spec.contentType="text/javascript";spec.text=aScript.source.text;}}catch(ex){}}
...
function reportError(aError,aPrefix=""){dbg_assert(aError instanceof Error,"Must pass Error objects to reportError");let msg=aPrefix+aError.message+":\n"+aError.stack;Cu.reportError(msg);dumpn(msg);}

function findCssSelector(ele){var document=ele.ownerDocument;if(ele.id&&document.getElementById(ele.id)===ele){return'#'+ele.id;} 
var tagName=ele.tagName.toLowerCase();if(tagName==='html'){return'html';}
if(tagName==='head'){return'head';}
if(tagName==='body'){return'body';}
if(ele.parentNode==null){console.log('danger: '+tagName);} 
var selector,index,matches;if(ele.classList.length>0){for(var i=0;i<ele.classList.length;i++){selector='.'+ele.classList.item(i);matches=document.querySelectorAll(selector);if(matches.length===1){return selector;}
...
index=positionInNodeList(ele,ele.parentNode.children)+1;selector=findCssSelector(ele.parentNode)+' > '+
tagName+':nth-child('+index+')';return selector;};function positionInNodeList(element,nodeList){for(var i=0;i<nodeList.length;i++){if(element===nodeList[i]){return i;}}
return-1;}
