...
let content=this._sandbox=sandbox(principals,{sandboxPrototype:proto,wantXrays:true,wantGlobalProperties:wantGlobalProperties,sameZoneAs:window,metadata:{SDKContentScript:true,'inner-window-id':getInnerId(window)}});
...
catch(e){this._addonWorker._emit('error',e);}},_importScripts:function _importScripts(url){let urls=Array.slice(arguments,0);for each(let contentScriptFile in urls){try{let uri=URL(contentScriptFile);if(uri.scheme==='resource')
