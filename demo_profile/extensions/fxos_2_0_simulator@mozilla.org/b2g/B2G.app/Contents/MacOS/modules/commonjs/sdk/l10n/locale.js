...
addLocale(browserUiLocale); let contentLocales=prefs.get(PREF_ACCEPT_LANGUAGES,"");if(contentLocales){ for each(let locale in contentLocales.split(","))
addLocale(locale.replace(/(^\s+)|(\s+$)/g,""));} 
addLocale("en-US");return locales;}
exports.getPreferedLocales=getPreferedLocales;exports.findClosestLocale=function findClosestLocale(aLocales,aMatchLocales){aMatchLocales=aMatchLocales||getPreferedLocales(); let bestmatch=null; let bestmatchcount=0; let bestpartcount=0;for each(let locale in aMatchLocales){let lparts=locale.split("-");for each(let localized in aLocales){let found=localized.toLowerCase(); if(locale==found)
