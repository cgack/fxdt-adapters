const Ci=Components.interfaces;const Cc=Components.classes;const Cr=Components.results;const Cu=Components.utils;const CACHE_MAX_GROUP_ENTRIES=100;Cu.import("resource://gre/modules/XPCOMUtils.jsm");function electrolify(service){
service.wrappedJSObject=service;var appInfo=Cc["@mozilla.org/xre/app-info;1"];if(appInfo&&appInfo.getService(Ci.nsIXULRuntime).processType!=Ci.nsIXULRuntime.PROCESS_TYPE_DEFAULT)
{ service._dbInit=function(){};}}
function ContentPrefService(){electrolify(this);
...
break;}}};const privModeStorage=new ContentPrefStore();ContentPrefService.prototype={ classID:Components.ID("{e3f772f3-023f-4b32-b074-36cf0fd5d414}"),QueryInterface:function CPS_QueryInterface(iid){let supportedIIDs=[Ci.nsIContentPrefService,Ci.nsIFrameMessageListener,Ci.nsISupports,];if(supportedIIDs.some(function(i)iid.equals(i)))
